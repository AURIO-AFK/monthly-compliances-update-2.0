    ...
    sectionHeadings.forEach((checkbox) => {
        checkbox.addEventListener('change', function () {
            ...
            while (nextRow && !nextRow.classList.contains('section-heading')) {
                if (this.checked) {
                    nextRow.classList.remove('faded');
                } else {
                    nextRow.classList.add('faded');
                }
                nextRow = nextRow.nextElementSibling;
            }
        });

        // Trigger the change event on load to set initial states
...
async function generatePDF() {
    ...
    const companyAddress = "123 Business Street, Kolkata, West Bengal";
    const companyContact = "Phone: +91-1234567890 | Email: contact@tosniwalassociates.com";
    ...
    // Draw a line below the letterhead
    doc.setDrawColor(0, 0, 0); // Black color for line
    doc.line(15, 38, 195, 38); // Draw a horizontal line across the page
    ...
    doc.autoTable({
        ...
        willDrawCell: (data) => {
            if (data.section === 'body') {
                const isSectionHeading = data.cell.raw.startsWith('1.') || 
                                         data.cell.raw.startsWith('2.') || 
                                         data.cell.raw.startsWith('3.') || 
                                         data.cell.raw.startsWith('4.') || 
                                         data.cell.raw.startsWith('5.') || 
                                         data.cell.raw.startsWith('6.') || 
                                         data.cell.raw.startsWith('7.') || 
                                         data.cell.raw.startsWith('8.') || 
                                         data.cell.raw.startsWith('9.');
